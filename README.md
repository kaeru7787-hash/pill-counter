...
**v0.3.0：開発用の実写5枚で、白錠90・袋入り丸形12・袋入り長円形36・黄色錠剤のトレー上12と36を検出。いずれも手動の正解中心との照合でFP0・FN0でした。** 改善に使った写真での成績であり、未知の写真の精度を示すものではありません。黄色錠剤の黒トレー写真も元写真で検証しています。学習済みAIモデルは付属せず、番号と数え漏れを必ず確認してください。完全に隠れた錠剤は写真から数えられません。
...
2. 撮影環境を選択します。透明袋では「透明な分包袋」を選んでください。着色錠剤の補助経路は環境の選択にかかわらず画像から適用を判断します。
...
公開リポジトリには患者情報・薬袋の印字を含む実写や書き出しデータをコミットしないでください。付属画像は合成17枚と、EXIFを除去して向きを補正した提供実写5枚です。
