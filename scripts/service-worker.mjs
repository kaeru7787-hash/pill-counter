...
event.respondWith(caches.open(CACHE).then(async cache=>{if(request.mode==='navigate'){const shell=await cache.match(new URL('index.html',ROOT).href,{ignoreVary:true});if(shell)return shell;}const cached=await cache.match(request,{ignoreVary:true});if(cached)return cached;try{const response=await fetch(request);if(response.ok&&response.type==='basic'&&!response.headers.get('content-type')?.includes('text/html'))await cache.put(request,response.clone());return response;}catch(error){if(url.pathname.endsWith('/models/pill-counter.onnx'))return new Response('',{status:404});throw error;}}));});`,
);
