...
v0.10.0は別系統の**点眼専用モデルを同梱**します。点眼の重み・学習・評価・ライセンスは [EYEDROP.md](EYEDROP.md)。以下は従来の錠剤モデルについての情報です。

v0.6はAIと従来CVを併用し、候補ごとの採否判定と局所再解析を行います。ONNXの重みはv0.5と同一です。実験した追加の特徴分類器は検証で不採用とし、実行コードには含めていません。写真を外部送信せず、端末内のONNX Runtime Web WASMで解析します。
